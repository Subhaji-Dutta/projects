...
String readFire()
{
  float f = digitalRead(Fire);
  if (isnan(f)) {
    Serial.println("Failed to read from fire sensor!");
    return "--";
  }
  else {
    Serial.println(f);
...
<body>
  ...
  <p>
    <i class="fas fa-tint" style="color:#00add6;"></i> 
    <span class="dht-labels">Fire </span>
...
setInterval(function ( ) {
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      document.getElementById("fire").innerHTML = this.responseText;
    }
  };
  xhttp.open("GET", "/fire", true);
  xhttp.send();
}, 10000 ) ;
...
void setup(){
  ...
  server.on("/smoke", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send_P(200, "text/plain", readSmoke().c_str());
  });
